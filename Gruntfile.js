...
module.exports = function(grunt) {
	...
	grunt.initConfig({
		...
		filename: 'adaptive',
		meta: {
			modules: 'angular.module("adaptive", [<%= srcModules %>]);',
	...
	//Common adaptive module containing all modules for src and templates
	...
	function findModule(name) {
		...
		grunt.file.expand('src/' + name + '/*.js').forEach(function(file) {
			srcModules.push(enquote('adaptive.' + name));
		});
	...
	function dependenciesForModule(name) {
		...
		.forEach(function(contents) {
			...
			dependencies.split(',').forEach(function(dep) {
				if (dep.indexOf('adaptive.') > -1) {
					var depName = dep.trim().replace('adaptive.','').replace(/['"]/g,'');
	...
	grunt.registerTask('build', 'Create adaptive build files', function() {
